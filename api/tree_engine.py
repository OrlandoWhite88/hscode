...
class HSCodeTree:
    ...
    def find_child_codes(self, parent_code: str) -> List[str]:
        ...
        if not parent_code:
            ...
            return [code for code in all_codes if re.match(r'^\d{2}
...
"""
        ...
        for child_code in child_codes:
            ...
            if child_node:
                direct_children.append({
                    ...
                    "general": child_node.general,
                    "special": child_node.special,
                    "other": child_node.other,
    ...
    def _get_full_context(self, code: str) -> str:
        ...
        if len(code) >= 2:  # Chapter level
            chapter_code = code[:2]
            chapter_node = self.tree.code_index.get(chapter_code)
            if chapter_node:
                path.append(f"{chapter_node.htsno} - {chapter_node.description}")
        
        if len(code) >= 4:  # Heading level
            heading_code = code[:4]
            if heading_code != code:  # Avoid duplicating the current node
                heading_node = self.tree.code_index.get(heading_code)
                if heading_node:
                    path.append(f"{heading_node.htsno} - {heading_node.description}")
        
        # Handle subheadings and tariff lines with periods
        parts = code.split('.')
        if len(parts) > 1:
            # Add each segment gradually
            current = parts[0]
            for i in range(1, len(parts)):
                current = f"{current}.{parts[i]}"
                if current != code:  # Avoid duplicating the current node
                    current_node = self.tree.code_index.get(current)
                    if current_node:
                        path.append(f"{current_node.htsno} - {current_node.description}")
...
, code)]
        ...
        # For 2-digit chapter codes, find 4-digit heading codes within that chapter
        if re.match(r'^\d{2}
...
if __name__ == "__main__":
    main()
        ...
        # For 4-digit heading codes, find subheading codes
        if re.match(r'^\d{4}
...
, parent_code):
        ...
        # For subheading codes, find tariff lines
